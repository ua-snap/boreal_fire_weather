...
def ffmc_calc(tas, pr, sfcWind, hurs, ffmc0: np.ndarray = 85.0) -> np.ndarray:
    ...
    ko = 0.424 * (1.0 - (hurs / 100.0) ** 1.7) + 0.0694 * np.sqrt(sfcWind) * (
        1.0 - (hurs / 100.0) ** 8.0
    )  # ..... Eq. 6a
    kd = ko * 0.581 * np.exp(0.0365 * tas)  # ............................. Eq. 6b

    k1 = 0.424 * (1.0 - ((100.0 - hurs) / 100.0) ** 1.7) + 0.0694 * np.sqrt(sfcWind) * (
        1.0 - ((100.0 - hurs) / 100.0) ** 8
    )  # ....... Eq. 7a
...
def isi_calc(ffmc, sfcWind) -> np.ndarray:
    ...
    fF = (
        91.9 * np.exp(-0.1386 * m) * (1.0 + (m**5.31) / (4.93 * 1e7))
    )  # .............................. Eq. 25
...
if __name__ == "__main__":
    ...
    with tqdm(
        ...
        for gcm in gcm_list:
            # Always use OUT_DIR/bias_corrected (output from 03 or 03b)
            ...
            for yr in cmip6_year_range:
                ...
                # Export/write CFFDRS results to netcdf file
                cffdrs_ds = cffdrs_ds.astype("float32")
                export_fn = Path(OUT_DIR).joinpath(
                    "cffdrs", gcm, f"cffdrs_{gcm}_{yr}.nc"
                )
                cffdrs_ds.to_netcdf(export_fn, engine="h5netcdf")

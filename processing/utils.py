...
def trim_geolims(ds: xr.Dataset, geolims: Iterable) -> xr.Dataset:
    ...
    sel_dict = {
        coord_names["lat"]: slice(latlim[0], latlim[1]),
...
def extract_str_parts(
    filelist: list, extension: str = "", sep: str = "_"
) -> np.ndarray:
    """
    Description
    -----------
...
def regrid_geodata(
    ds: xr.Dataset, target_coords, method="linear", **kwargs
) -> xr.Dataset:
    """
...
def raster_transform_to_coords(rst_file) -> tuple:
    ...
    for i in range(n):
        coords_i = np.arange(
            origin[i] + res[i] / 2, origin[i] + res[i] / 2 + size[i] * res[i], res[i]
        )

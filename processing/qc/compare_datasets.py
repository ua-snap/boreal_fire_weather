...
def compare_datasets(file1: Path, file2: Path) -> tuple:
    ...
    for var in data_vars1:
        ...
        if arr1.shape != arr2.shape:
            all_identical = False
            messages.append(f"  Variable '{var}': Shape mismatch ({arr1.shape} vs {arr2.shape})")
            continue
        ...
        if not np.array_equal(nan_mask1, nan_mask2):
            ...
            messages.append(f"  Variable '{var}': NaN patterns differ ({nan_diff} locations)")
...
def run_qc(old_dir: str, new_dir: str, variables: list = None, max_files: int = None):
    ...
    if variables:
        var_groups = {var: files for var, files in var_groups.items() if var in variables}
        if not var_groups:
    ...
    print(f"Running QC on {total_files} files across {len(var_groups)} variable(s)...\n")
...
Examples:
  ...
  python compare_datasets.py /data/old /data/new "" 10
    ...
    parser.add_argument(
        'old_dir',
        help='Path to reference/old dataset directory'
    )
    parser.add_argument(
        'new_dir',
        help='Path to new dataset directory to validate'
    )
    parser.add_argument(
        'variables',
        nargs='?',

...
def quantile_delta_mapping(
    ...
    **kwargs: additional keyword arguments to be passed on to various functions
    ...
    QDM = QuantileDeltaMapping.train(ref,hst,
        **get_kwargs(('group','nquantiles','kind'),kwargs))        
    ...
    if return_hst:
        ...
        hst_ba = QDM.adjust(hst,
            **get_kwargs(('interp','extrapolation'),kwargs))
        ...
        for gcm in gcm_list:
            ...
            for var in metvars:
                for sim_year in sim_periods:
                    ref_src = [list(era5_dir.glob("%s*%d*" % (var,i)))[0] for i in range(hist_years[0],hist_years[1]+1)]
                    hst_src = [list(in_dir.glob("%s*%d*" % (var,i)))[0] for i in range(hist_years[0],hist_years[1]+1)]
                    sim_src = [list(in_dir.glob("%s*%d*" % (var,i)))[0] for i in range(sim_year[0],sim_year[1]+1)]

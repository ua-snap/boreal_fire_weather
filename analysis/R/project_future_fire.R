...
files <- list.files(path = "data/model_results/jags-results",
                    pattern = "\\.RData$")
...
for (mdl in src) {
    ...
    for (f in seq_along(files)) {
        ...
        file_i <- files[f]
        jags_results <- load(file.path("data/model_results/jags-results", 
                                       file_i))
        ...
        for (yr in pred_yrs) {
            ...
            # AIS added step
            # subtract the 2 km2 added to all AAB values in model fit stage 
            # to get rid of 0's
            aab_pred_i <- aab_pred_i - 2

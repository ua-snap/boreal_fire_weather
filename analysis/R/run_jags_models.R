...
suppressPackageStartupMessages(library(dplyr)) #ais added
suppressPackageStartupMessages(library(future)) #ais added
suppressPackageStartupMessages(library(future.apply))  #ais added
...
results_dir <- "data/model_results/jags-results" #ais added "jags-results" subfolder
...
# AIS replacing lines above. To keep bias consistent, add 2km2 to all 
# observed burns so that we don't have 0's. Only fires > 2km2 were included 
# in our observed burns dataset
df <- df %>%
    rename(aab_old = aab) %>%
    mutate(aab = aab_old+2) #add small number to all values to avoid 0 AAB years
...
plan(multisession)  # or plan(multicore) for Linux/Mac

# For each ecoregion, run the Bayesian model in parallel
results <- future_lapply(seq_along(unique_ecos), function(i) {
          print(paste0("working on ecos ",unique_ecos[i]))
    ...
    for (j in seq_along(jags_models)) {
        print(paste0("Model: ", j, " for ecos: ", unique_ecos[i]))
        ...
        for (feedback in feedback_type) {
            ...
            # Add years covered in modeling to jags output for export
            jags_output$model$year <- df_i$year
            
            export_fn <- file.path(results_dir,
                                   sprintf("jags-results_%s_%s-model_%d.RData", distr, feedback, unique_ecos[i]))
            
            # Save output as RData file
            save(jags_output, file = export_fn)
            
            # Store the model output in the list
            model_results[[feedback]] <- jags_output 
        }
    }
    return(model_results)  # Return the results for this ecoregion
})
...
#   # For each model (here using a Gamma distributions)
#   for (j in seq_along(jags_models)) {
#       print(paste0("iteration ",j," out of ",nrow(jags_models),
#                    "for ecos ",unique_ecos[i]))
# 
#     jags_model_header <- readLines(jags_models[j], n = 2)
#     distr <- strsplit(jags_model_header[[1]], " ")[[1]][-c(1, 2)]
#     params_to_save <- strsplit(jags_model_header[[2]], " ")[[1]][-c(1, 2)]

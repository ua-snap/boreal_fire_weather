...
def _max_calc(ds: xr.Dataset,hst_yr=(1980,2009)) -> xr.Dataset:
    ...
    # Find annual maximum
    ds_annual_max = ds.groupby('time.year').max(dim='time', skipna=True) #ais

    # Find 30-yr average annual maximum
    ds_max_hst_avg = ds_annual_max.sel(year=yr_slice).mean(dim="year", skipna=True) #ais
...
def _ndays_gt_95th(ds: xr.Dataset,hst_yr: tuple=(1980,2009)) -> xr.Dataset:
    ...
    ds_95pct_hst = ds.sel(time=yr_slice).quantile(0.95,dim='time', skipna=True) #ais
    ...
    ds_95d = ds_95pct_rel.groupby('time.year').sum(dim='time', skipna=True) #ais
...
def _fs(ds: xr.Dataset) -> xr.Dataset:
    ...
    ds_fs = ds.rolling(time=90,center=True).mean(skipna=True) #ais
    ds_fs = ds_fs.groupby('time.year').max(skipna=True) #ais
...
def _fwsl(ds: xr.Dataset,hst_yr: tuple=(1980,2009)) -> xr.Dataset:
    ...
    ds_min = ds.sel(time=yr_slice).min(dim='time',skipna=True) #ais
    ds_max = ds.sel(time=yr_slice).max(dim='time',skipna=True) #ais

    ds_norm = 100.0 * ((ds - ds_min) / (ds_max - ds_min))
    
    ds_fwsl = xr.where(ds_norm > 50.0,1.0,0.0)
    ds_fwsl = ds_fwsl.groupby('time.year').sum(dim='time', skipna=True) #ais 

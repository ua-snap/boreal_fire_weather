...
def mask_arrays(ref: xr.DataArray,
    ...
    if mask is not None:
        ...
        if isinstance(mask,np.ndarray):
            ...
            mask_vct = gpd.read_file(mask)

            # Calculate spatial extents
            mask_bounds = mask_vct.total_bounds
            ref_bounds = (ref.lon.min().item(), ref.lat.min().item(), ref.lon.max().item(), ref.lat.max().item())

            # Check overlap of extents
            mask_bounds_poly = shapely.geometry.box(*mask_bounds)
            ref_bounds_poly = shapely.geometry.box(*ref_bounds)

            if not mask_bounds_poly.intersects(ref_bounds_poly):
                print("Mask and ref extents do not overlap. Reprojecting mask to EPSG:4326.")
                mask_vct = mask_vct.to_crs(epsg=4326)
            ###ais

            mask_grd = h.mask_from_shp(mask_vct,ref)
...
def chunk_data_arrays(ref,hst,sim,frac=0.2):
    ...
    n, m = (ref[axes['X']].size, ref[axes['X']].size) #ref[axes['X'][0]].size, ref[axes['X'][0]]
...
def quantile_delta_mapping(
    ...
    **kwargs: additional keyword arguments to be passed on to various functions
    ...
    ref = xr.open_mfdataset(
        ...
        engine='h5netcdf',
        parallel=dask_load#,
        #chunks={'time':365,'lon':-1,'lat':-1} ais
        )
    ...
    # Convert to NumPy Array - this is what allows QDM.train to work!!
    ref = ref.compute() #ais
    hst = hst.compute() #ais
    sim = sim.compute() #ais 

    # Training step in Quantile delta mapping
    QDM = QuantileDeltaMapping.train(ref=ref,hist=hst,  

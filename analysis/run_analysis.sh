...
# conda create --name boreal-wildfire-analysis python=3.9.15 r-base=4.4.3 -c conda-forge 

# source ~/miniconda3/etc/profile.d/conda.sh ais wron local file path
conda activate boreal-wildfire-analysis

# ais installed these individually to avoid downgrading some packages. 
# ais used conda list to see which packages were already installed
# conda install -c conda-forge \
  ...
  # numpy \
  # pandas \
  # pyproj \ #involves downgrading
  # pyyaml \
  # rasterio \ #involves downgrading
  # shapely \
  # tqdm \
  # xarray \
  # xclim
...
EARTHDATA_TOKEN="v1OR-taBvhQ4k0VwkR8ziDzMM6h_pECGSEE_qAIrvL42c_bSKp2iRE8sA3ymMRvTNI4eij2d-ttCf2Iw4MNB4A" 
#"[NEED TO INSERT EARTHDATA LOGIN TOKEN HERE]" 
#ais token expires only a few days
...
download_ecoregion_shapefiles.sh #ais script didn't work, but opening the files manually did

# Download and organize fire data
downlad_fire_data.sh #ran manually
rasterize_fire_shapefiles.sh #ran manually

# Download and reproject MOD44B datasets
request_mod44b.sh $EARTHDATA_TOKEN #able to collect files on AppEARS, but had to download manually
reproject_mod44b.sh
